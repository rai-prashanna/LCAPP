...
valid_solution(M) -> valid_rows(M) andalso valid_rows(transpose(M)) andalso 
valid_rows(blocks(M)).
...
triples_parallel([Row]) -> [triples(Row)];
triples_parallel([Row1|Tail]) -> 
Parent=self(),Tag=make_ref(),
spawn_link(fun()->
Parent!{Tag,triples_parallel(Tail)}end),
receive {Tag,Ys} -> [triples(Row1)|Ys] end.
...
blocks(M) ->
  Inter_Transposed= transpose(triples_parallel(M)),
  Blocks=triples_parallel(Inter_Transposed),
