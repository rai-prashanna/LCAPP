...
prefine_row([Head]) ->refine_row([Head]);
prefine_row([Head|Tail]) ->
Parent=self(),Tag=make_ref(),
spawn_link(fun()->
Parent!{Tag,prefine_row(Tail)}end),
receive {Tag,Ys} -> [refine_row(Head)|Ys] end.


prefine_rows(no_solution) ->
  no_solution;
prefine_rows(M) ->
  Refined = prefine_row(M),
...
pzip(A,B) -> lists:reverse(pziph(A,B,[])).
pziph([],L2, Z) -> Z;
pziph(L1,[], Z) -> Z;
pziph([H1|L1],[H2|L2], Z) ->
pziph(L1,L2,[{H1,H2}|Z]).
